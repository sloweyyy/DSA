...
int main()
{
    ...
    cout << "Thoi gian tai du lieu: " << chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - start).count() << "ms" << endl;
    int type;
    cout << "Chon chuc nang: " << endl;
    cout << "1. Tim kiem bang mang bam." << endl;
    cout << "2. So sanh hieu suat tim kiem 1000 tu giua mang bam, binary search va binary tree search" << endl;
    cin >> type;
    string word;
    switch (type)
    {
    case 1:
        ...
        while (1)
        {
            ...
            cout << "Thoi gian tim: " << chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - start).count() << "ms" << endl;
        }
        break;
    case 2:
        int total = 0;
        ...
        for (int i = 0; i < 1000; i++)
        {
            ...
            total += chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - start).count();
        }
        cout << "Thoi gian tim 1000 tu bang mang bam: " << total << "ms" << endl;
        total = 0;
        NodeTree *root = NULL;
        start = chrono::high_resolution_clock::now();
        for (int i = 0; i < n; i++)
        ...
        cout << "Thoi gian tim random 1000 tu bang cay: " << total << "ms" << endl;
        // tim bang mang  + binary search
        total = 0;
        start = chrono::high_resolution_clock::now();
        for (int i = 0; i < 1000; i++)
        {
            binarySearch(data[randomIndex[i]].word, data, n);
            total += chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - start).count();
        }
        cout << "Thoi gian tim random 1000 tu bang binary search: " << total << "ms" << endl;
        break;
    }
    delete[] data;
    system("pause");
